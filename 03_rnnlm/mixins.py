...
class BoardRecorderMixin:
    ...
    def build_step_time_reocrder(self):
        self._last_time = tf.placeholder(tf.float64, name='last_time')
        self._currnet_time = tf.placeholder(tf.float64, name='current_time')
        self._step_run = tf.placeholder(tf.float64, name='step_run')
        self._per_step = tf.placeholder(tf.float64, name='per_step')
        tf.summary.scalar(
            'Step_Time',
    ...
    def open_writer(self, log_dir):
        ...
        return tf.summary.FileWriter(log_dir, tf.get_default_graph())
